...
function findBadImports(files) {
    return files.map(filePath => {
        ...
        const badImports = file.split('\n')
            ...
            .map(row => ({line: row.line, text: row.text.substr(row.text.indexOf(' from ') + 6).replace(/'|;|"/g, '')}))
            ...
            .map(row => {
                ...
                const bad = globby.sync([
                    ...
                    path.resolve(source, row.text + '/index.d.ts'),
                ]);

                if (bad.length < 1) {
                    console.log(path.resolve(source, row.text));
                    return {
                        shortImportPath: row.text,
                        fullImportPath: path.resolve(source, row.text +  '.ts'),
                        line: row.line
                    };
                }
